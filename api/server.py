...
def require_paper_mode():
    if settings.nxio_env not in ("paper", "development"):
        ...
        raise HTTPException(status_code=403, detail="Real trading not enabled. Set NXIO_ENV=paper in .env")
...
class BuyRequest(BaseModel):
    ...
    sl:              float | None = Field(None)
    tp:              float | None = Field(None)
...
def health():
    ...
    models_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "brain", "models")
    model_files = []
    ...
    return {
        "status":     "online" if checks["database"] == "ok" else "degraded",
        "system":     "NXIO",
        "version":    "3.0.0",
...
def get_candles(asset: str = Query(default="RELIANCE"), tf: str = Query(default="5m"), n: int = Query(default=60)):
    ...
    if not tf_cfg:
        return {"error": f"Unknown timeframe: {tf}"}
    path = os.path.join(PROC_DIR, tf_cfg["label"], f"{asset}_features.csv")
    if not os.path.exists(path):
        return {"error": f"No data for {asset} {tf_cfg['label']}"}
    df = pd.read_csv(path, index_col="Datetime", parse_dates=True).tail(n)
    candles = []
    for dt, row in df.iterrows():
        candles.append({
            "datetime": str(dt), "open": round(float(row.get("Open", 0)), 2),
            "high": round(float(row.get("High", 0)), 2), "low": round(float(row.get("Low", 0)), 2),
            "close": round(float(row.get("Close", 0)), 2), "volume": int(row.get("Volume", 0)),
    ...
    return {"asset": asset, "tf": tf_cfg["label"], "count": len(candles), "candles": candles}
...
def get_predict(asset: str = Query(default="RELIANCE"), tf: str = Query(default="5m"), candles: int = Query(default=5)):
    ...
    return {"asset": asset, "timeframe": tf, "timestamp": datetime.now().strftime("%d %b %Y %H:%M:%S"),
...
def paper_buy(req: BuyRequest):
    require_paper_mode()
    return buy_stock(symbol=req.symbol, price=req.price, quantity=req.quantity,
...
def paper_history():
    ...
    return {"history": history, "total": len(history), "wins": len(wins), "losses": len(losses),
